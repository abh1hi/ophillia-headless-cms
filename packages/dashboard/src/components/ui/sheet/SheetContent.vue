...
const delegated = computed(() => { const { class: _, side: __, ...rest } = props; return rest })
...
<template>
  <DialogPortal>
    ...
    <DialogContent
      v-bind="forwarded"
      :class="cn(
        'fixed z-50 gap-4 bg-background p-6 shadow-xl transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500 data-[state=open]:animate-in data-[state=closed]:animate-out flex flex-col',
        sideClasses[side],
        props.class
      )"
    >
